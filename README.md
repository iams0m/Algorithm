# Algorithm
### 백준 알고리즘 문제풀이와 기본적인 알고리즘 및 자료구조에 대한 학습 내용을 기록하였습니다.<br>
...
- 점화식 : 어떤 함수를 자신보다 더 작은 변수에 대한 함수와의 관계로 표현한 것
      ...
      - D[i][j] = D[i - 1][j] + D[i - 1][j - 1]
         ...
         - D[i - 1][j] : 어떤 특정한 원소를 포함하지 않고 뽑은 경우

         - D[i - 1][j - 1] : 어떤 특정한 원소를 포함하여 뽑은 경우
...
`동적 계획법의 원리`
    ...
    - 모든 작은 문제들은 한 번만 계산해 DP 테이블에 저장하며, 추후 재사용 할 때는 이 DP 테이블을 이용한다. ➡️ memoization
...
`동적 계획법 구현 방식`
   ...
   1. 동적 계획법으로 풀 수 있는지 확인하기
      ...
      - 앞서 살펴본 `동적 계획법의 원리`에 따라 동적 계획법으로 풀 수 있는지 확인
 
   2. 점화식 세우기

      - 논리적으로 전체 문제를 나누고, 전체 문제와 부분 문제 간의 인과 관계를 파악하여 점화식 세우기

   3. 메모이제이션(memoization) 원리 이해하기

      - `memoization` : 부분 문제를 풀었을 때 이 문제를 DP 테이블에 저장해두고 다음에 같은 문제가 나왔을 때 다시 계산하지 않고 DP 테이블 값을 이용하는 것
        ➡️ 불필요한 연산과 탐색이 줄어들어 시간 복잡도 측면에서 많은 이점을 가질 수 있음
